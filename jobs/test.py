...
def main():
    ...
    schema = StructType([
        StructField("t", StringType(), False),        # Exchangesegment_ExchangeInstrumentID(exchange segment enum with underscore along with instumentID of the particular subscribed instrument)
        StructField("87", DoubleType(), False),      # Last traded price
        StructField("145", DoubleType(), False),       # Total price volume
    ])
    ...
    try:
        tickerDf = read_kafka_topic('market_data_topic', schema)
